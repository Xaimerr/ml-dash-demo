    ...
    [
        ...
        html.Div(
            [
                html.Div(
                    [
                        html.H4("Received Signal Strength Indicator vs time", className="app__header__title"),
                        html.P(
                            "This app continually queries a SQL database and displays live charts of RSSI and RALA state.",
                ...
                html.Div(
                    [
                        html.Img(
                            src=app.get_asset_url("drexel.png"),
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        html.Div(
                            [html.H6("RSSI [dB]", className="graph__title")]
                        ),
                        dcc.Graph(
                            id="metric",
                            figure=dict(
                        ...
                        html.Div(
                            [
                                html.Div(
                                    [
                                        html.H6(
                                            "Radiation pattern", className="graph__title"
                                        )
                                    ]
                                ),
                                dcc.Graph(
...
@app.callback(
    Output("metric", "figure"), [Input("metric-update", "n_intervals")]
)
def gen_wind_speed(interval):
    ...
    df = get_wind_data(total_time - 20, total_time)
    ...
    layout = dict(
        ...
        yaxis={
            ...
            "nticks": max(6, round(df["rssi"].iloc[-1] / 10)),
        },
    )
...
@app.callback(
    Output("wind-direction", "figure"), [Input("metric-update", "n_intervals")]
)
def gen_wind_direction(interval):
    ...
    val = 20
    direction = [0, ((df["state"][0]-1)*90 - 45), ((df["state"][0]-1)*90 + 45), 0]
...
def gen_wind_histogram(interval, wind_speed_figure, slider_value, auto_state):
    ...
    try:
        # Check to see the metric has been plotted yet
...
def deselect_auto(slider_value, metric_figure):
    """ Toggle the auto checkbox. """

    # prevent update if graph has no data
    if "data" not in metric_figure:
        raise PreventUpdate
    if not len(metric_figure["data"]):
        raise PreventUpdate

    if metric_figure is not None and len(metric_figure["data"][0]["y"]) > 5:
        return [""]
    return ["Auto"]
